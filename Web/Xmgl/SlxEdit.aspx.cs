...
namespace Web.Xmgl
{
    public partial class SlxEdit : TStar.Web.BasePage
    {
        #region 自定义属性
        ...
        private void BindData()
        {
            ...
            // 绑定项目类别
            TUF.Helper.BindTreeView(BLL.Globals.SystemCode.DtTree_khzbLocalEdit, this.ddlZbbh, "id", "－请选择－");
            this.ddlZbbh.SelectedValue = Zbbh;
            this.ddlZbbh_SelectedIndexChanged(null, null);
            this.lblZbmc.Text = this.ddlZbbh.SelectedText;

            //this.ShowTxsm("");

            this.dtpXmrq.MinDate = DateTime.Now.AddMonths(-3);//DateTime.Parse("2013-01-01");
        ...
        private void ShowUI()
        {
            ...
            if (!IsAdd)
            {
                ...
                // 项目基本信息
                Xmbh = xm.Pkid;
                this.ddlZbbh.SelectedValue = xm.Zbbh;
                this.ddlZbbh_SelectedIndexChanged(null, null);
                this.ddlDjbh.SelectedValue = xm.Djbh;
                this.dtpXmrq.SelectedDate = DateTime.Parse(xm.Xmrq);
                this.tbxLxdx.Text = xm.Xmmc.Substring(11, xm.Xmmc.Length - 15);
        ...
        private void SaveXmxx()
        {
            ...
            string lxdx = TU.Globals.FilterString(this.tbxLxdx.Text);
            string xmmc = "";
            ...
            if (this.ddlDjbh.SelectedIndex < 1)
            {
                errMsg += "请选择联系类别 ！\n";
            }
            if (String.IsNullOrEmpty(lxdx))
            {
                flag = false;
                errMsg += "请输入联系对象 ！\n";
            }
            ...
            if (flag)
            {
                xmmc = dtXmrq.Value.ToString("yyyy年MM月dd日") + "联系" + lxdx;
        ...
        public string UploadFile(string xmbh, int no, HttpPostedFile f)
        {
        ...
        protected void ddlZbbh_SelectedIndexChanged(object sender, EventArgs e)
        {
            string zbbh = this.ddlZbbh.SelectedValue;

            // 绑定填写说明
            DataRow dr = TU.Globals.FindRow(BLL.Globals.SystemCode.DtJd_khzbLocal.DefaultView, "Pkid", zbbh);
            string txsm = dr["Txsm"].ToString();
            this.ShowTxsm(txsm);
            string yxsc = dr["Yxsc"].ToString();
            this.tbxBt1.Required = this.tbxBt1.ShowRedStar = yxsc == "True";

            // 绑定项目等级
            string filter = String.Format("Zbbh IN ('__','{0}')", zbbh);
            TUF.Helper.BindDropDownList(BLL.Globals.SystemCode.DtJd_xmdjLocal, this.ddlDjbh, "Djmc", "Pkid", null, filter, "__", "－请选择联系类别－");
            this.ddlDjbh.Hidden = this.ddlDjbh.Items.Count <= 1;
        }
